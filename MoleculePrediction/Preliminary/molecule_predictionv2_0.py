...
class molecule_prediction_data_wrapper:
    ...
    def get_input_vector(self, molecule):
        ...
        if molecule in self.molecule_mulliken_set:
            ...
            df = df.drop(columns=['atom'])
        padded_adm = pad(self.distance_matrices[molecule], (30, 30), (0, 0))
        atom_array = np.pad(df.values, [(0, 30 - df.shape[0]), (0, 1024-df.shape[1])], 'constant', constant_values=(0.0, 0.0))
